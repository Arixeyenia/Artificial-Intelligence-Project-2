...
def get_enemy_pieces_in_blast_range(board, origin):
    all_enemy_stack = []
    all_enemy_stack = range_check_get_enemy_pieces(board, origin)
    
    for a_stack in all_enemy_stack:
        more_stacks = range_check_get_enemy_pieces(
            board, a_stack.get_coordinates())
...
def get_ally_pieces_in_blast_range(board, origin):
    all_ally_stack = []
    all_ally_stack = range_check_get_ally_pieces(board, origin)
...
def get_stack_closest_to_enemy(board, turn):
    ...
    for a_stack in player.values():
        ...
        for e_stack in player2.values():
            distance_to_all_enemies += manhattan_distance(
                a_stack.get_coordinates(), e_stack.get_coordinates())
